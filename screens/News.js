...
const News = ({ route, navigation }) => { 
  ...
  useEffect(() => {
    // MODIFIED: Ensure moreInformation is correctly extracted from doc.data()
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach(doc => {
        ...
        items.push({ 
          ...
          moreInformation: data.moreInformation, // <--- CRITICAL FIX: Ensure this line is present
  ...
  const renderNewsItem = ({ item }) => (
    <TouchableOpacity 
      ...
      onPress={() => navigation.navigate('NewsDetail', { newsItem: item, isAdmin })} 
    >
      {item.imageUrl ? ( 
        <Image 
        ...
        <ScrollView contentContainerStyle={styles.singleNewsContainer}>
          <View style={[styles.newsCardFull, { backgroundColor: newsItem.backgroundColor || '#f0f4ff' }]}>
            ...
            <View style={styles.newsContentFull}>
              ...
              {/* This is the crucial part for full moreInformation display */}
