...
const PartnerMsg = () => {
  ...
  // Ref to keep track of conversations for which a notification has been sent
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        const conversationsPromises = snapshot.docs.map(async (d) => {
          ...
          // Push notification logic for new partner messages
          if (isUnreadForCurrentUser && !notifiedPartnerConvosRef.current.has(d.id) && !isFocused) {
            console.log(`NEW PARTNER MESSAGE DETECTED for convo: ${d.id}`);
            const itAgentsQuery = query(
              collection(db, 'users'),
              where('role', '==', 'IT'), // Assuming IT agents handle partner messages
            ...
            itAgentsSnap.forEach(agentDoc => {
              if (agentDoc.id !== currentUser.uid) { 
                const agentData = agentDoc.data();
                if (agentData.expoPushToken) {
                  sendPushNotification(
                    agentData.expoPushToken,
                    `Nouveau message de ${partnerDisplayName}`,
                    conversationData.lastMessage || "Un partenaire vous a envoyé un nouveau message.",
                    { type: 'partner_message', partnerId: d.id, partnerName: partnerDisplayName }
                  );
