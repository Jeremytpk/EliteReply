  ...
  useEffect(() => {
    const fetchUserCoupons = async () => {
      ...
      try {
        ...
        // MODIFIED QUERY: Add condition for isRedeemed == false
        const q = query(
          collection(db, 'surveyResult'),
          where('userId', '==', currentUser.uid),
          where('isRedeemed', '==', false), // <--- ADD THIS LINE
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          // Ensure couponDetails and qrCodeData exist before adding
          if (data.couponDetails && data.qrCodeData) {
        ...
        // This query (where + where + orderBy) likely needs a new composite index.
        // The error message from Firebase will tell you exactly what index to create.
        Alert.alert('Erreur', 'Impossible de charger vos coupons. Veuillez réessayer ou vérifier la console pour créer un index Firestore.');
      } finally {
        setLoading(false);
      }
    };
